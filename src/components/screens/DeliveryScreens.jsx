...
export function PodScreen({ order, photo, otp, onPhoto, onOtp, onVerify }) {
  ...
  return (
    <>
      ...
      <div className="photo" aria-live="polite">
        {photo ? (
          ...
          <div className="photo-empty">
            <svg width="30" height="26" viewBox="0 0 30 26" aria-hidden="true">
              <rect x="1" y="5" width="28" height="20" rx="4" stroke="#B4AFA5" strokeWidth="2" fill="none" />
              <path d="M10 5 L12 1 H18 L20 5" stroke="#B4AFA5" strokeWidth="2" fill="none" strokeLinejoin="round" />
              <circle cx="15" cy="15" r="5.5" stroke="#B4AFA5" strokeWidth="2" fill="none" />
            </svg>
            <span>{t("No photo of the unloaded material yet")}</span>
          </div>
        )}
      </div>
    ...
    return (
      <>
        ...
        <div className="card flat" style={{ textAlign: "center", padding: 20 }}>
          ...
          <div className="k" style={{ marginTop: 3 }}>{t("Cash collected from {name}", { name: order.cust.name })}</div>
        </div>
        <div className="card">
          <div className="rowline"><span>{t("Order")}</span><b>{order.id}</b></div>
          <div className="rowline"><span>{t("Mode")}</span><b>{t(order.upiPaid ? "Cash and UPI" : "Cash on delivery")}</b></div>
          <div className="rowline"><span>{t("Cash in your pocket")}</span><b>{rs(order.collected || order.amount)}</b></div>
        </div>
  ...
  return (
    <>
      ...
      <div className="card flat">
        <div className="rowline"><span>{t("Material value")}</span><b>{rs(order.amount)}</b></div>
        <div className="rowline"><span>{t("Delivery")}</span><b>{t("Free above ₹500")}</b></div>
        <div className="rowline"><span>{t("Cashback to customer")}</span><b>{rs(Math.round(order.amount * 0.01))}</b></div>
      </div>
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="rowline"><span>{t("Delivered in")}</span><b>{t("{n} min of 60", { n: Math.round((order.deliveredAt - order.placedAt) / 60000) })}</b></div>
      </div>
